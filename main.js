      ...
      for (let i = 0; i < categoryData.results.length; i++) {
        ...
        try {
          ...
          statusEl.textContent = `Lade ${
            detailData.name || item.index
          } aus ${categoryName} (${i + 1}/${categoryData.results.length})`;
        } catch (err) {
...
function displayCategories() {
  ...
  document.getElementById('delete-data').style.display = liveMode
    ? 'none'
    : 'inline-block';
  document.getElementById('download-data').textContent = liveMode
    ? 'daten herunterladen'
    : 'daten neu laden';
}
  ...
  if (Array.isArray(obj)) {
    ...
    for (const [key, value] of Object.entries(obj)) {
      if (key === 'url' && typeof value === 'string') {
        ...
        if (!['item', 'from'].includes(key)) {
          const label = document.createElement('div');
          label.className = 'content-info';
          label.textContent = key.toString().replace(/_/g, ' ');
          subWrapper.appendChild(label);
        }
        renderJSON(value, subWrapper, level + 1);
        wrapper.appendChild(subWrapper);
      } else if (
        !['index', 'option_type', 'updated_at', 'name'].includes(key) &&
        typeof value != 'object'
      ) {
        ...
        info.innerHTML = `${key
          .toString()
          .replace(/_/g, ' ')}: ${marked.parse(
          value.toString().replace(/_/g, ' ')
        )}`;
        wrapper.appendChild(info);
      } else if (key == 'updated_at' && typeof value != 'object') {
        const info = document.createElement('div');
        info.className = 'content-info';
        info.textContent = `${key
          .toString()
          .replace(/_/g, ' ')}: ${getTimeString(value)}`;
        wrapper.appendChild(info);
      } else if (key == 'name' && typeof value != 'object') {
        const info = document.createElement('div');
        info.className = 'content-info';
        info.innerHTML = `<span class="name-container">${marked.parse(
          value.toString().replace(/_/g, ' ')
        )}</span>`;
        wrapper.appendChild(info);
      }
...
async function navigateToURL(url) {
  ...
  if (!liveMode) {
    ...
    if (
      ...
      currentData[category].results[fullDetailPath]
    ) {
      ...
      console.log(
        'Verfügbare Keys:',
        Object.keys(currentData[category]?.results || {})
      );
    }
  } else {
...
function debugLoading(percentage) {
  ...
  if (loadingContainer && mainContent && progressBar && progressContainer) {
    ...
    console.error(
      'Ein oder mehrere erforderliche Elemente wurden nicht gefunden.'
    );
  }
}
